...
public:
    rope() = default;
    rope(const std::string& s) : root_(std::make_shared<Node>(s)) {}

    rope& append(const rope& r)
    {
        if (!root_) { root_ = r.root_; return *this; }
        if (!r.root_) return *this;
        root_ = std::make_shared<Node>(root_, r.root_);
        return *this;
    }

    size_t size() const { return root_ ? root_->len : 0; }
    ...
    rope& operator+=(const rope& other) { return append(other); }

private:
    void append_to_string(const std::shared_ptr<Node>& n, std::string& out) const
    {
        if (!n) return;
        if (!n->left && !n->right) { out += n->s; return; }
