...
public:
    ...
    void swap(queue& other) noexcept(noexcept(std::declval<container_type&>().swap(other.c_))) { c_.swap(other.c_); }

    template <class U, class C> friend bool operator== (const queue<U,C>& lhs, const queue<U,C>& rhs) { return lhs.c_ == rhs.c_; }
    template <class U, class C> friend bool operator!= (const queue<U,C>& lhs, const queue<U,C>& rhs) { return lhs.c_ != rhs.c_; }
    template <class U, class C> friend bool operator<  (const queue<U,C>& lhs, const queue<U,C>& rhs) { return lhs.c_ < rhs.c_; }
    template <class U, class C> friend bool operator<= (const queue<U,C>& lhs, const queue<U,C>& rhs) { return lhs.c_ <= rhs.c_; }
    template <class U, class C> friend bool operator>  (const queue<U,C>& lhs, const queue<U,C>& rhs) { return lhs.c_ > rhs.c_; }
